...
/*   Updated: 2025/03/12 15:43:06 by dmelnyk          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
/**
 * @brief Reads a complete line of input, handling multi-line continuation.
 *
 * This function repeatedly calls `readline` to get input lines. It handles
 * multi-line input by checking for unclosed quotes or parentheses using
 * `is_there_missing_char`.  It concatenates the lines together, adding a
 * newline character between them.  It also handles SIGINT during input.
 *
 * @param data  A pointer to the `t_data` structure.
 * @param state A pointer to the `t_quote_state` structure to track quoting.
 */
